...
class User extends Authenticatable
{
    ...
    public function permittedMenus()
    {
        ...
        $roleMenus = $this->role?->menus ?? collect();
        ...
        if ($this->hasPermissionPivotTable('menu_user')) {
            return $this->menus()->wherePivot('can_view', true)->get();
        }
    ...
    public function permittedSubmenus()
    {
        ...
        $roleSubmenus = $this->role?->submenus ?? collect();

        if ($roleSubmenus->isNotEmpty()) {
            return $roleSubmenus;
        }

        if ($this->hasPermissionPivotTable('submenu_user')) {
            return $this->submenus()->wherePivot('can_view', true)->get();
        }
    ...
    public function canSubmenuAction(string $action, ?string $routeName = null): bool
    {
        ...
        $hasRoleAccess = $this->role?->submenus()->where('submenus.IdSubMenu', $submenu->IdSubMenu)->exists() ?? false;
        if ($hasRoleAccess) {
            return true;
        }
